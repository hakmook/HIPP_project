#### Figure 6D, Secretion traits, hormone contents, and cell compositions with predicted ancestry (scaled) ####
...
for (var in INS_vars) {
  model <- as.formula(paste(var, "~ SuperPopulationClass + Donor_HbA1c_s + Gender + Age_years_s + center + BMI_s + PreShipmentCultureTime + IsletTransitTime"))
  model0 <- as.formula(paste(var, "~ Donor_HbA1c_s + Gender + Age_years_s + center + BMI_s + PreShipmentCultureTime + IsletTransitTime"))
