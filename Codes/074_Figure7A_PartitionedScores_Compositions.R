...
for (m1 in 1:length(Part_GRS_vars)){
  ...
  for (m in 1:length(cell_vars)){
    model = as.formula(paste0(cell_vars[m], "~", Part_GRS_vars[m1]," + Donor_HbA1c_s  + Gender +  Age_years_s + center + BMI_s + 
  ...
  rownames(Summary_Table[[1]][[m1]]) = cell_vars
}
...
model_out2 = vector(mode='list', length=length(cell_vars))


for (m1 in 1:length(Part_GRS_vars2)){
  temp_summary = NULL
  for (m in 1:length(cell_vars)){
    model = as.formula(paste0(cell_vars[m], "~", Part_GRS_vars2[m1]," + Donor_HbA1c_s  + Gender +  Age_years_s + center + BMI_s + 
               PreShipmentCultureTime + IsletTransitTime + PC1 + PC2 + PC3 + PC4 + PC5"))
    fit = lm(model, data = dat_all)
    model_out2[[m]][[m1]] = fit 
    temp = summary(fit)
  ...
  rownames(Summary_Table[[2]][[m1]]) = cell_vars
}
