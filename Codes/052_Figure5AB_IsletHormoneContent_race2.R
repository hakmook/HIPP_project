...
for (m in 1:length(outcome_int_ins)){
  model = as.formula(paste0(outcome_int_ins[m], "~", 
                            "Islet_Insulin_Content_ng_IEQ_s", " + Gender + Donor_HbA1c_s + race2 + Age_years_s + center + BMI_s + 
...
for (m in 1:length(outcome_int_ins)){
  model = as.formula(paste0(outcome_int_ins[m], "~", 
                            "Islet_Glucagon_Content_pg_IEQ_s", " + Gender + Donor_HbA1c_s + race2 + Age_years_s + center + BMI_s + 
               PreShipmentCultureTime + IsletTransitTime"))
  fit = lm(model, data = hipp)
  temp = summary(fit)
  temp1 = temp$coefficients[2,]
