#### Figure 5, Insulin and Glucagon Secretion Traits with cell composition (scaled) ####
...
for (m in 1:length(outcome_int_ins)){
  model = as.formula(paste0(outcome_int_ins[m], "~", 
                            "Beta_cell", " + Gender + Donor_HbA1c_s + race2 + Age_years_s + center + BMI_s + 
...
for (m in 1:length(outcome_int_ins)){
  model = as.formula(paste0(outcome_int_ins[m], "~", 
                            "Alpha_cell", " + Gender + Donor_HbA1c_s + race2 + Age_years_s + center + BMI_s + 
               PreShipmentCultureTime + IsletTransitTime"))
  fit = lm(model, data = hipp)
  temp = summary(fit)
  temp1 = temp$coefficients[2,]
...
for (m in 1:length(outcome_int_ins)){
  model = as.formula(paste0(outcome_int_ins[m], "~", 
                            "Delta_cell", " + Gender + Donor_HbA1c_s + race2 + Age_years_s + center + BMI_s + 
               PreShipmentCultureTime + IsletTransitTime"))
  fit = lm(model, data = hipp)
  temp = summary(fit)
  temp1 = temp$coefficients[2,]
