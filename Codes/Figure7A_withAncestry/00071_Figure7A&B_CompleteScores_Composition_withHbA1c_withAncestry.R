#### Figure 7A, Complete scores - Compositions, Cell compositions with GRS T1D and T2D UPDATED (scaled) ####
...
for (m in 1:length(cell_vars)){
  model = as.formula(paste0(cell_vars[m], "~ T1D.GRS +  Donor_HbA1c_s + Gender + race2 + Age_years_s + center + BMI_s + 
...
temp_summary = NULL
for (m in 1:length(cell_vars)){
  model = as.formula(paste0(cell_vars[m], "~ T2D.GRS +  Donor_HbA1c_s + Gender + race2 + Age_years_s + center + BMI_s + 
               PreShipmentCultureTime + IsletTransitTime"))
  fit = lm(model, data = dat_all)
  model_out_g[[m]] = fit
  temp = summary(fit)
...
######################################
#### With Predicted Ancestry #####
######################################


dat_all$SuperPopulationClass = as.factor(dat_all$SuperPopulationClass)

#################################################
############ GRS T1D & T2D
# insulin secretion 
Summary_Table_sp = vector(mode='list', length=2)
model_out = vector(mode='list', length=length(cell_vars))

temp_summary = NULL
for (m in 1:length(cell_vars)){
  model = as.formula(paste0(cell_vars[m], "~ T1D.GRS +  Donor_HbA1c_s + Gender + SuperPopulationClass + Age_years_s + center + BMI_s + 
               PreShipmentCultureTime + IsletTransitTime"))
  fit = lm(model, data = dat_all)
  model_out[[m]] = fit 
  temp = summary(fit)
...
temp_summary$adj_pvalue = formatC(p.adjust(temp_summary[,4], method='fdr'),
                                  format = "e", digits = 3)
Summary_Table_sp[[1]] = temp_summary

rownames(Summary_Table_sp[[1]]) = cell_vars
...
temp_summary = NULL
for (m in 1:length(cell_vars)){
  model = as.formula(paste0(cell_vars[m], "~ T2D.GRS +  Donor_HbA1c_s + Gender + SuperPopulationClass + Age_years_s + center + BMI_s + 
               PreShipmentCultureTime + IsletTransitTime"))
  fit = lm(model, data = dat_all)
  model_out_g[[m]] = fit
  temp = summary(fit)
...
temp_summary$adj_pvalue = formatC(p.adjust(temp_summary[,4], method='fdr'),
                                  format = "e", digits = 3)
Summary_Table_sp[[2]] = temp_summary

rownames(Summary_Table_sp[[2]]) = cell_vars

names(Summary_Table_sp) = c("GRS T1D:cell comp", "GRS T2D:cell comp")
...
Summary_Tab = Summary_Table_sp[[1]][,-c(2,3)]
colnames(Summary_Tab) = c("Coeff", "P-val", "Adj P-val")
Summary_Tab


### GRS T2D and cell compositions
Summary_Tab = Summary_Table_sp[[2]][,-c(2,3)]
colnames(Summary_Tab) = c("Coeff", "P-val", "Adj P-val")
Summary_Tab
