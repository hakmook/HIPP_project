...
for (m in 1:length(cell_vars)){
  model = as.formula(paste0(cell_vars[m], "~ T1D.GRS + Donor_HbA1c_s + Gender +  Age_years_s + center + BMI_s + 
...
temp_summary = NULL
for (m in 1:length(cell_vars)){
  model = as.formula(paste0(cell_vars[m], "~ T2D.GRS + Donor_HbA1c_s + Gender +  Age_years_s + center + BMI_s + 
               PreShipmentCultureTime + IsletTransitTime + PC1 + PC2 + PC3 + PC4 + PC5"))
  fit = lm(model, data = dat_all)
  model_out_g[[m]] = fit
  temp = summary(fit)
